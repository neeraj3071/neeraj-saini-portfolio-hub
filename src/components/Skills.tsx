...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-24 bg-background relative overflow-hidden">
      {/* Stickers */}
      <div className="absolute top-10 right-10 text-4xl float-sticker sticker rounded-xl bg-white p-2">🛠️</div>
      <div className="absolute bottom-10 left-10 text-4xl float-sticker-reverse sticker rounded-xl bg-white p-2">⚙️</div>
        ...
        <div className="grid lg:grid-cols-12 gap-5">
          {skillCategories.map((category) => {
            ...
            return (
              <div
                key={category.title}
                className={`${category.span} ${category.bg} ${category.tilt} brutal-border brutal-shadow rounded-brutal p-6 bouncy`}
              >
